...
export function createTabStatePersistence(host: Pick<HostAPI, 'services'>, key: string): AddonTabPersistence {
  return {
    load: async () => host.services.get<AddonStateStore>('addonStateStore')?.get<AddonTabViewState>(key),
    save: async (state) => {
      await host.services.get<AddonStateStore>('addonStateStore')?.set(key, state);
    },
  };
}
