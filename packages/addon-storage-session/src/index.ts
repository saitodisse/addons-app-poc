...
export const manifest = {
  ...
  services: [
    { id: 'addonStateStore', version: '1.0.0', name: 'Add-on State Store', description: 'Armazena estados serializáveis no sessionStorage' },
  ],
  interactions: {
    version: '1.0.0',
    services: [{ id: 'addonStateStore', role: 'provides', description: 'Guarda estado serializável somente durante a sessão da aba.', methods: [{ id: 'get', description: 'Lê um estado por chave.' }, { id: 'set', description: 'Grava um estado por chave.' }, { id: 'remove', description: 'Remove um estado por chave.' }, { id: 'listKeys', description: 'Lista as chaves do protocolo.' }, { id: 'clear', description: 'Remove todos os estados do protocolo.' }] }],
...
export function setup(host: HostAPI): void {
  host.registerService('addonStateStore', createSessionStateStore());
