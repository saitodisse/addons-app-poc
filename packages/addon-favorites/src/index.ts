import type { AddonStateStore, AddonTab, Bookmark, BookmarkStore, HostAPI } from '@addons/core';
...
export function createOptionalStateFavoritesService(host: Pick<HostAPI, 'services'>) {
  ...
  return {
    list: read,
    async add(title: string, url?: string): Promise<Bookmark> {
...
export function setup(host: HostAPI): void {
  host.registerService('favorites', createOptionalStateFavoritesService(host));
  host.log('info', 'Add-on favorites configurado com sucesso');
}

export function createTab(host: HostAPI): AddonTab {
  const favorites = host.services.get<ReturnType<typeof createFavoritesService>>('favorites');
  return {
    ...manifest.tab,
    fields: [
    ...
    async run(actionId, values) {
      ...
      if (actionId === 'add') {
        ...
        const favorite = await favorites.add(title, values.url?.trim() || undefined);
